"""
Archiv-Stub: Diese Datei wurde als veraltet markiert und ihr Inhalt archiviert.
Verwende zur Entwicklungszeit `ocr_fastapi_single.py` im selben Ordner oder die
kopierten Versionen in `server/dev/`.

Die Datei ist bewusst minimal, um Mehrfachimplementierungen zu vermeiden.
"""

raise RuntimeError("Deprecated: use server/ocr_fastapi_single.py or server/dev/ocr_fastapi_clean.archived.py")
    ...
    for p in candidates:
        ...
        if os.path.exists(mod_path):
            ...
            try:
                ...
                # import may fail (e.g. SyntaxError) - try next candidate
                continue
...
def _import_ocr_module():
    """Versucht, `ocr_engine` aus einigen typischen Pfaden zu importieren."""
    try:
        return importlib.import_module('ocr_engine')
    except Exception:
        pass
    ...
    for p in candidates:
        ...
        if os.path.exists(mod_path):
            ...
            try:
                ...
                # Import kann fehlschlagen (z.B. SyntaxError) — zum nächsten Kandidaten
                continue
...
async def api_ocr(file: UploadFile = File(...)):
    ...
    try:
        data = await file.read()
        # process_pdf_bytes erwartet bytes
        res = process_pdf_bytes(data, filename=filename)
...
# Try to import the OCR module from a few likely locations (no code changes to OCR required)
def _import_ocr_module():
    try:
        import ocr_engine
        return ocr_engine
    ...
    # Candidate relative paths (project layout dependent)
    here = os.path.dirname(__file__)
    candidates = [
        os.path.abspath(os.path.join(here, '..', '..', 'Archiv')),
        os.path.abspath(os.path.join(here, '..', '..')),
...
try:
    ...
    async def api_ocr(file: UploadFile = File(...)):
        ...
        try:
            data = await file.read()
            # call the OCR processor (may be CPU/time intensive)
            # pass default dpi (300) — the processor supports dpi and max_pages parameters
            res = process_pdf_bytes(data, filename=filename)
            ...
            tb = traceback.format_exc()
            # map OCR processing errors to 422
            return JSONResponse(status_code=422, content={"code": "ocr_error", "message": str(e), "trace": tb})
